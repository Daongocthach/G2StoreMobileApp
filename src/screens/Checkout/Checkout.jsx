...
const Checkout = () => {
    ...
    var total = cart?.total ? cart.total + feeShip - voucherValue : 0
    if (total < 0)
        total = 0
    ...
    function handleClickPromotion() {
        promotionApi.checkPromotion(code)
            .then(response => {
                setVoucherValue(response.data.value)
                total = total - response.data.value
                showAlertOk()
            })
            .catch(err => {
                console.log(err)
    ...
    useEffect(() => {
        if (user && Array.isArray(cart?.cartItems)) {
            let totalHeight = 0
            let totalLength = 0
            let totalWeight = 0
            let totalWidth = 0
            cart.cartItems.forEach(cartItem => {
                totalHeight += cartItem.product.height || 0
                totalLength += cartItem.product.length || 0
                totalWeight += cartItem.product.weight || 0
                totalWidth += cartItem.product.width || 0
            })
            ghnApi.calculateFeeShip(user.districtId, totalHeight, totalLength, totalWeight, totalWidth)
                .then((response) => {
    ...
    return (
            <SafeAreaView style={styles.container}>
                        ...
                        <View style={styles.body}>
                            ...
                            <View className='w-11/12 bg-white rounded-md p-1 mb-1'>
                                ...
                                <View className='flex-row items-center gap-2 mt-2'>
                                    ...
                                    <TextInput className='border-b-2 border-gray-500 w-40' placeholder='Nhập mã ...' placeholderTextColor={'gray'} onChangeText={setCode} value={code} />
                                ...
                                <View className='flex-row justify-between items-center'>
                                    ...
                                    <Text className='text-base font-medium text-red-500'>- {formatCurrency(voucherValue)}</Text>
                                </View>
                                <View className='flex-row justify-between items-center'>
                ...
                <View style={styles.footer}>
                    ...
                    <Text className='text-xl text-red-500 font-semibold'>{formatCurrency(total)}</Text>
